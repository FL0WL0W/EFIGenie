...
namespace Operations
{
	...
	{
        EngineScheduleInjectionArray ret = EngineScheduleInjectionArray(_length);

        for(uint8_t i = 0; i < _length; i++)
        {
            std::tuple<ScalarVariable, ScalarVariable> ex = _array[i]->Execute(enginePosition, ignitionDwell, ignitionAdvance);
            ret.OpenTick[i] = std::get<0>(ex);
            ret.CloseTick[i] = std::get<1>(ex);
        }
	...
	Operations::IOperationBase *Operation_EngineScheduleInjectionArray::Create(Service::ServiceLocator * const &serviceLocator, const void *config, unsigned int &sizeOut)
	{
        ...
        for(uint8_t i = 0; i < length; i++)
            ignitionOutputOperation[i] = reinterpret_cast<Operations::IOperation<void, ScalarVariable> *>(IOperationBase::Create(serviceLocator, config, sizeOut));

		return new Operation_EngineScheduleInjectionArray(timerService, length, tdc, ignitionOutputOperation);
	}

	IOPERATION_REGISTERFACTORY_CPP(Operation_EngineScheduleInjectionArray, 2006, EngineScheduleInjectionArray, EnginePosition, ScalarVariable, ScalarVariable)
}
#endif