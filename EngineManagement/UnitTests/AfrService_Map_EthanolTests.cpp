...
{		
	...
	public:
		...
		ApplicationService::IAfrService *_afrService;
		...
		{		
			...
			*((unsigned char *)buildConfig) = 2;
			...
			unsigned int size = 0;
			_afrService = ServiceBuilder::CreateAfrService(_serviceLocator, config, &size);
		}
		...
		TEST_METHOD(WhenGettingAfrThenCorrectAfrIsReturned)
		{
			...
			_afrService->CalculateAfr();
			Assert::AreEqual(11.3034375f, _afrService->Afr, 0.1f);
			Assert::AreEqual(0.769f, _afrService->Lambda, 0.01f);
			...
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(11.93140625f, _afrService->Afr, 0.1f);
			Assert::AreEqual(0.81f, _afrService->Lambda, 0.01f);

			EXPECT_CALL(_timerService, GetTick()).Times(1).WillOnce(Return(50000));
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(12.559375f, _afrService->Afr, 0.1f);
			Assert::AreEqual(0.855f, _afrService->Lambda, 0.01f);
			...
			_afrService->CalculateAfr();
			Assert::AreEqual(11.76f, _afrService->Afr, 0.1f);
			Assert::AreEqual(0.8f, _afrService->Lambda, 0.01f);

			_mapService.Value = 0.165;
			_ectService.Value = 100;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(1000));
			_afrService->CalculateAfr();
			Assert::AreEqual(15.14f, _afrService->Afr, 0.1f);

			_mapService.Value = 0.165;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(2000));
			_afrService->CalculateAfr();
			Assert::AreEqual(14.9f, _afrService->Afr, 0.1f);

			_mapService.Value = 0;
			_tpsService.Value = 1;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(12.94f, _afrService->Afr, 0.1f);

			_ethanolService.Value = 1;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(8.79f, _afrService->Afr, 0.1f);

			_tpsService.Value = 0;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(10.55f, _afrService->Afr, 0.1f);

			_ethanolService.Value = 0.5;
			EXPECT_CALL(_decoder, GetRpm()).Times(1).WillOnce(Return(0));
			_afrService->CalculateAfr();
			Assert::AreEqual(12.97f, _afrService->Afr, 0.1f);
		}
