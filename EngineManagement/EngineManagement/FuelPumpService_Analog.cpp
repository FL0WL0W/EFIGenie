...
{
	FuelPumpService_Analog::FuelPumpService_Analog(const FuelPumpService_AnalogConfig *config, ITimerService *timerService, IFloatOutputService *outputService, IDecoder *decoder, IFloatInputService *manifoldAbsolutePressureService, IFloatInputService *throttlePositionService)
	{
		_config = config;
	...
	void FuelPumpService_Analog::Prime()
	{
		...
		_timerService->ScheduleTask(&FuelPumpService_Analog::PrimeTaskOff, this, _config->PrimeTime + _timerService->GetTick(), true);
	}
	...
	{	
		...
		_currentValue = InterpolateTable2<unsigned char>(_decoder->GetRpm(), _config->MaxRpm, 0, _config->RpmRes, y, _config->MaxY, 0, _config->YRes, _config->AnalogTable);
