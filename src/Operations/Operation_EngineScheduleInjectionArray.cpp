...
{
	Operation_EngineScheduleInjectionArray::Operation_EngineScheduleInjectionArray(EmbeddedIOServices::ITimerService *timerService, uint8_t length, const float *tdc, IOperation<void, ScalarVariable> **ignitionOutputOperation)
	{
        _length = length;
	...
	IOperationBase *Operation_EngineScheduleInjectionArray::Create(Service::ServiceLocator * const &serviceLocator, const void *config, unsigned int &sizeOut)
	{
	...
	IOPERATION_REGISTERFACTORY_CPP(Operation_EngineScheduleInjectionArray, 2008)
}
#endif