...
namespace UnitTests
{
	class AcceptanceTest : public ::testing::Test 
	{
		...
		AcceptanceTest()
		{
			...
			EXPECT_CALL(_digitalService, AttachInterrupt(_, _)).WillOnce(SaveArg<1>(&_crankTriggerCallback));
			EXPECT_CALL(_digitalService, AttachInterrupt(_, _)).WillOnce(SaveArg<1>(&_camTriggerCallback));
